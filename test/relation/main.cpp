...
void test_match_empty_list() {
  ...
  auto wrapped_non_empty_list = store.make_input_destructured_tuple(non_empty_list);

  MT_ERROR_IF_UNRELATED(eq, "", empty_rvalue_tup, wrapped_non_empty_list);
  MT_ERROR_IF_UNRELATED(eq, "", empty_rvalue_tup, non_empty_list);
}
...
void test_subtyping() {
  ...
  const auto sub_d_handle = library.get_sub_number_type().value();
...
void test_equivalence_debug() {
  ...
  const auto d_handle = library.get_number_type().value();
  const auto c_handle = library.get_char_type().value();
  ...
  auto empty_rvalue_tup = store.make_rvalue_destructured_tuple(TypePtrs{});
  auto wrapped_non_empty_list = store.make_input_destructured_tuple(store.make_list(d_handle));
